...
class Trainer(object):
    ...
    def train_model(self):
        ...
        for epoch in range(self.epochs):
            ...
            # if epoch % 100 == 0:
            #     logger.info(f" before epoch %d the best iteration is %d, results: %s" %
            #                 (epoch, best_epoch + 1, best_dict.__str__()))
        # save the best model
    ...
    def _train_one_epoch(self, behavior_dataset_loader, epoch):
        ...
        logger.info('%d epoch %.2fs Train loss is [%.4f] ' % (epoch + 1, epoch_time, total_loss))
        ...
        epoch_time = time.time() - start_time
        logger.info(
            f"%d validate cost time %.2fs, result: %s " % (epoch + 1, epoch_time, validate_metric_dict.__str__()))
        ...
        epoch_time = time.time() - start_time
        logger.info(
            f"%d test cost time %.2fs, result: %s " % (epoch + 1, epoch_time, test_metric_dict.__str__()))
